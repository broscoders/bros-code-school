...
export default function TeacherAssignments() {
  ...
  const [form, setForm] = useState({ classId: "", sectionId: "", subjectId: "", title: "", instructions: "", totalMarks: "", dueDate: "" });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    await api.post("/ops/assignments", { ...form, schoolId, teacherId: teacher._id });
  ...
  return (
    <div className="p-8">
      ...
      <form onSubmit={handleSubmit} className="bg-surface rounded-xl border border-black/5 shadow-sm p-5 mt-6 grid grid-cols-2 gap-3">
        ...
        <select value={form.subjectId} onChange={(e) => setForm({ ...form, subjectId: e.target.value })} className="border border-black/10 rounded-md px-3 py-2 text-sm col-span-2" required>
        ...
        <textarea placeholder="Instructions" value={form.instructions} onChange={(e) => setForm({ ...form, instructions: e.target.value })} className="border border-black/10 rounded-md px-3 py-2 text-sm col-span-2" rows={2} />
        <input type="number" placeholder="Total Marks" value={form.totalMarks} onChange={(e) => setForm({ ...form, totalMarks: e.target.value })} className="border border-black/10 rounded-md px-3 py-2 text-sm" />
      ...
      <div className="space-y-3 mt-6">
        {list.length === 0 && <p className="text-muted text-sm">Select a class to see assignments.</p>}
        {list.map((a) => (
