  ...
  return (
    <div className="p-8">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-6">
        {cards.map((c) => (
          <div key={c.label} className="bg-surface border border-slate-800 rounded-xl p-4">
            ...
            <tbody>
              {stats.recentOrganizations.map((org: any) => (
                <tr key={org._id} className="border-t border-slate-800">
                  ...
                  <td className="p-3 text-muted">{org.status}</td>
                  <td className="p-3 text-muted text-xs">{new Date(org.createdAt).toLocaleDateString()}</td>
                </tr>
              ))}
            </tbody>
          </table>
