...
export default function Admissions() {
  ...
  const load = async () => {
    const res = await api.get(`/extra/admissions?schoolId=${schoolId}`);
    setList(res.data);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    await api.post("/extra/admissions", { ...form, schoolId });
    setForm({ applicantName: "", parentName: "", parentContact: "", desiredClassId: "", academicSystem: "" });
    load();
  };

  const updateStatus = async (id: string, status: string) => {
    await api.put(`/extra/admissions/${id}/status`, { status });
    load();
  };
