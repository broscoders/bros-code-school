      ...
      <div className="grid grid-cols-3 gap-4 mt-6">
        <div className="bg-surface rounded-2xl border border-border shadow-sm p-5">
          ...
          <form onSubmit={addBuilding} className="space-y-2 mb-3">
            ...
            <select value={buildingForm.type} onChange={(e) => setBuildingForm({ ...buildingForm, type: e.target.value })} className="w-full border border-border rounded-lg px-3 py-2 text-sm">
              <option value="BOYS">Boys</option>
              <option value="GIRLS">Girls</option>
            </select>
            <input placeholder="Warden Name" value={buildingForm.wardenName} onChange={(e) => setBuildingForm({ ...buildingForm, wardenName: e.target.value })} className="w-full border border-border rounded-lg px-3 py-2 text-sm" />
        ...
        <div className="bg-surface rounded-2xl border border-border shadow-sm p-5">
          ...
          <form onSubmit={addRoom} className="space-y-2 mb-3">
            ...
            <input placeholder="Room Number" value={roomForm.roomNumber} onChange={(e) => setRoomForm({ ...roomForm, roomNumber: e.target.value })} className="w-full border border-border rounded-lg px-3 py-2 text-sm" required />
            <input type="number" placeholder="Capacity" value={roomForm.capacity} onChange={(e) => setRoomForm({ ...roomForm, capacity: e.target.value })} className="w-full border border-border rounded-lg px-3 py-2 text-sm" required />
        ...
        <div className="bg-surface rounded-2xl border border-border shadow-sm p-5">
          ...
          <form onSubmit={allocate} className="space-y-2 mb-3">
            ...
            <select value={allocForm.roomId} onChange={(e) => setAllocForm({ ...allocForm, roomId: e.target.value })} className="w-full border border-border rounded-lg px-3 py-2 text-sm" required>
              <option value="">Select Room</option>
              {rooms.map((r) => <option key={r._id} value={r._id}>Room {r.roomNumber} ({r.occupied}/{r.capacity})</option>)}
            </select>
            <input type="number" placeholder="Monthly Fee" value={allocForm.monthlyFee} onChange={(e) => setAllocForm({ ...allocForm, monthlyFee: e.target.value })} className="w-full border border-border rounded-lg px-3 py-2 text-sm" required />
